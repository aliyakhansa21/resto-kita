...
function MenuContent() {
    ...
    useEffect(() => {
        ...
        bootstrap();
    }, [urlToken, urlTable, urlName, router]);
    ...
    // GATEKEEPER SESI MEJA
    const isMissingToken = isSessionReady && !tableToken;
    const isBackendSessionError = isError && error?.message?.toLowerCase().includes("tablesession");
    ...
    return (
        ...
        <main>
            ...
            >
                {/* Pengecekan status inisialisasi sesi */}
                {!isSessionReady && (
                ...
                {/* Sesi Tidak Valid (Frontend Gatekeeper ATAU Backend Error) */}
                ...
                {/* TAMPILKAN KONTEN HANYA JIKA SESI VALID */}
                {!showInvalidSessionUI && isSessionReady && (
                    <>
                        {/* Filter row */}
                        ...
                        {isError && !isBackendSessionError && (
                            <div className="flex flex-col items-center justify-center py-20 px-4">
                                <div className="bg-white rounded-3xl shadow-lg border border-red-100 max-w-md w-full p-8 text-center">
                                    ...
                                    <h2 className="text-xl font-bold text-stone-800 mb-2">Gagal Memuat Menu</h2>
                                    <p className="text-stone-500 text-sm leading-relaxed mb-6">Terjadi kesalahan saat mengambil data menu. Periksa koneksi internet kamu dan coba lagi.</p>
                                    <button onClick={() => window.location.reload()} className="w-full py-3 px-6 rounded-2xl bg-red-500 hover:bg-red-600 active:scale-95 text-white font-semibold text-sm transition-all duration-150 shadow-md shadow-red-200">Muat Ulang Halaman</button>
                                </div>
                            </div>
                        )}
        ...
        {/* Floating Cart Button — Hanya muncul jika ada item DAN Sesi Valid */}
        {totalItems > 0 && !showInvalidSessionUI && (
