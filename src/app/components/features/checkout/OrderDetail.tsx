import type { CheckoutSession, PaymentMethod, CartItem } from "@/types";
...
export function OrderDetail({ session, cartSnapshot, paymentMethod, onBack }: Props) {
    ...
    return (
        <div className="bg-[#FAF7F2] rounded-2xl shadow-md overflow-hidden">
            ...
            {/* Payment Info */}
            <section className="px-6 py-5 border-secondary-20">
                <h2 className="font-bold text-base text-primary-50 mb-4">
                🧾 Payment Information
                </h2>
                <div className="bg-[#F7F7F6] rounded-xl overflow-hidden">
                    <div className="flex justify-between items-center px-4 py-3 border-secondary-20">
                    ...
                    <div className={`flex justify-between items-center px-4 py-3 ${!isCash ? "border-secondary-20" : ""}`}>
                        <span className="text-text-xs font-semibold text-secondary-50 uppercase tracking-wider">
                        Method
                        </span>
            ...
            {/* Items */}
            <section className="px-6 py-5 border-secondary-20">
                <h2 className="font-bold text-base text-[#0F172A] mb-4">Items</h2>
                <div className="flex flex-col divide-y divide-secondary-20">
                    {cartSnapshot.map((ci) => (
                        <div key={ci.menuItem.id} className="flex items-center gap-4 py-3">
                            <img
                                src={
                                ci.menuItem.imageUrl ||
                                `https://placehold.co/56x56/8b5e3c/ead7c5?text=${ci.menuItem.name[0]}`
                                }
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                {ci.quantity}x {fmt(ci.menuItem.price)}
                                </p>
                            </div>
                            <p className="font-bold text-base text-[#0F172A] flex-shrink-0">
                                {fmt(ci.menuItem.price * ci.quantity)}
                            </p>
                        </div>
                    ))}
                </div>
                ...
                {/* Totals */}
                    <div className="border-secondary-20 pt-4 mt-3 flex flex-col gap-2">
                    ...
                    <div className="flex justify-between font-black text-h4 text-[#0F172A] border-secondary-20 pt-3 mt-1">
            ...
            <div className="flex justify-end gap-4 px-6 pb-7">
                <button className="border-2 border-primary text-primary font-bold text-base rounded-full py-3 px-8 hover:bg-primary hover:text-white transition-all">
                Download Receipt
                </button>
                <button
                onClick={onBack}
                className="bg-primary text-white font-bold text-base rounded-full py-3 px-8 hover:bg-primary-10 transition-colors"
                >
                Back to Home
                </button>
            </div>
